...
Ce projet implémente un modèle de régression en Machine Learning pour une prédiction basique du prix des actions basé sur l'architecture Long Short-Term Memory (LSTM), un type de réseau de neurones récurrents (RNN). Le modèle est développé en utilisant la bibliothèque PyTorch et est capable de prédire le prix de clôture futur d'une action à partir de ses prix passés.
